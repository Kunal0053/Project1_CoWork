...
app.post("/worker/register", upload.fields([{ name: "document", maxCount: 1 }, { name: "photo", maxCount: 1 }]), async (req, res) => {
  try {
    const existing = await Worker.findOne({ email: req.body.email });
...
app.get("/workers/:skill", async (req, res) => {
  try {
    const workers = await Worker.find({ skill: req.params.skill, status: "approved" });
    res.json(workers);
  } catch {
    res.status(500).json({ message: "Error fetching workers" });
  }
...
app.post("/book", async (req, res) => {
  try {
    ...
    await Worker.findOneAndUpdate(
      { name: req.body.workerName },
      { $inc: { earnings: 200, gigs: 1 } }
    );
    // Return bookingId so frontend can rate it later
...
app.post("/rate-booking", async (req, res) => {
  try {
    ...
    const newAverage = (sumRatings / totalRatings).toFixed(1);
